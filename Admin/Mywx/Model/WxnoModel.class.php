...
class WxnoModel extends Model{
	public function readNoList($where = ' 1 = 1 '){
		...
		if ($params['wx_name']!=''){
			...
			$where .= " and t_n.wx_name like '%".$params['wx_name']."%'";
		}
		if ($params['wx_no']!=''){
		...
		$where .= " and t_n.wx_id in (".session('admin.wxids').")";
		...
		$row_list = $this->table('wx_wxno as t_n')
			...
			->join('wx_wxno_category as t_nt on t_n.cat_id = t_nt.cat_id')
			->where($where)
			->order('t_n.is_recommend desc,t_n.is_hot desc,t_n.wx_order desc,t_n.add_time desc')
