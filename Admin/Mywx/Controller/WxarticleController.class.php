...
class WxarticleController extends Controller{
	...
	function getContentByUrl($url){
		$Emoji = new \Wxno\Controller\EmojiController();
		...
		$art_desc = $Emoji->emoji_to_string($art_desc);
		
		// $title_content = mb_convert_encoding($title_content, 'UTF-8');
		$title_content = $Emoji->emoji_to_string($title_content);
		
		$art_content = str_replace('data-src="','src="',$contents);
		$art_content = $Emoji->emoji_to_string($art_content);
		return array(
	...
	public function artRushM(){
		...
		for ($i=0; $i < count($aids); $i++) { 
			if ($aids[$i]>0){
				...
				/*dump(htmlspecialchars_decode($art_real_link['art_real_link']));
				exit();*/
	...
	public function artAdd(){
		...
		if ($_GET['aid']>0){
			$artInfo = D('Wxarticle')->readartList('t_a.aid = '.$_GET['aid']);
			if (!is_array($artInfo['list'][0]) || count($artInfo['list'][0])<=0){
				$this->error('参数错误，请检查文章ID！');
				exit();
			}
		...
		$data_wxartSave = array(
			'data' => array(
				'art_title' => str_replace('"', "'", $params['new_art_title']),
				'wx_id' => $params['wx_id'],
		...
		if ($params['save']=='1'){
			...
			if (false===$res_artSave){
				...
				$this->Init->logRec($data_log);
				$this->success('公众号文章信息更新成功！',U('Mywx/Wxarticle/artList'));
			}
			...
			if ($res_artAdd>0){
				...
				$this->Init->logRec($data_log);
				$this->success('公众号文章信息添加成功！',U('Mywx/Wxarticle/artList'));
			}else{
	...
	public function artAddM(){
		...
		if ($params['save']=='1'){
			...
			if ($res_count==0){
				...
				$this->Init->logRec($data_log);
				$this->success('共添加<'.$art_count.'>篇文章，其中有<'.($art_count-$res_count).'>篇添加失败！',U('Mywx/Wxarticle/artList'));
			}else{
				$data_log = array(
					'log_type' => 2,	//1登录;2操作
					'user_name' => session('admin.member_name'),
				...
				$this->Init->logRec($data_log);
				$this->success('文章批量添加成功，共添加<'.$art_count.'>篇文章！',U('Mywx/Wxarticle/artList'));
			}
		}else{
			$this->wxnoList = D('Wxno')->readNoAll();
