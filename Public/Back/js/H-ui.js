...
$(function(){
	...
	*/
	$(document).on("change",".input-file",function(){
		var uploadVal = $(this).val();
		var pic_ext = /\.[^\.]+/.exec(uploadVal);	//后缀 .jpg ・・・・・
		var ext = pic_ext.toString().toLowerCase(); //转小写
		var arr_ext = '.jpg.png.gif.bmp';
		if ((typeof(uploadVal) != "undefined") && (uploadVal != '')){
			if (arr_ext.indexOf(ext)==-1){
				layer.alert('请选择正确图片上传！<br />仅支持‘jpg/png/gif/bmp’格式图片！',{icon:0});
				$(this).attr('title','未选择文件');
				$(this).parent().find(".upload-url").val('');
			}else{
				$(this).attr('title',uploadVal);
				$(this).parent().find(".upload-url").val(uploadVal).focus().blur();
			}
		}else{
			$(this).parent().find(".upload-url").val('');
